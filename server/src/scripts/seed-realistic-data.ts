...
const GENERAL_USERNAMES = [
    "adrian", "alice", "allen", "amber", "bruno", "carson", "diana", "elena", "felix", "grace",
    "hansel", "irene", "jared", "karen", "kevin", "louis", "mason", "nolan", "oliver", "peter",
    "quinn", "rachel", "samuel", "taylor", "ursula", "victor", "warren", "xavier", "yvette", "zack",
    "minseo", "jiho", "seojin", "haeun", "taehyun", "jisoo", "sujin", "hyerin", "junseo", "yujin",
    "seungmin", "haemin", "donghyun", "chaewon", "yeji", "sangwoo", "hyunjin", "gyuri", "jinwoo", "hyejin",
    "eunji", "siwoo", "doyun", "arin", "nayoung", "woojin", "seungah", "dayoon",
] as const;

const ENGLISH_FOCUS_USERS = new Set<string>(["oliver", "quinn", "samuel"]);

const KOREAN_TITLE_TOPICS = [
    "배포", "업로드", "로그인", "보안", "세션", "게시판", "공지", "질문", "성능", "운영",
    "백업", "점검", "장애", "리팩터링", "정책", "가이드", "문서", "모니터링",
] as const;

const KOREAN_TITLE_ACTIONS = [
    "점검", "공유", "요청", "후기", "정리", "계획", "문의", "수정", "확인", "메모",
] as const;
...
const ENGLISH_TITLE_TOPICS = [
    "Release", "Upload", "Security", "Session", "Board", "QnA", "Ops", "Monitoring", "Auth", "Policy",
] as const;

const ENGLISH_TITLE_ACTIONS = [
    "update", "check", "note", "review", "question", "follow-up", "summary", "status",
] as const;
...
const HOUR_WEIGHTS = [
    2.8, 2.6, 1.1, 0.8, 0.3, 0.3, 0.35, 0.55, 0.9, 1.2, 1.45, 1.8,
    3.0, 2.85, 1.95, 1.7, 1.7, 1.8, 2.0, 2.35, 3.0, 2.9, 2.7, 2.45,
] as const;
...
function parseArgs(): ParsedArgs {
    ...
    return {
        ...
        auditCount: Math.max(
            40_000,
            Math.min(60_000, toInt(auditCountArg ? auditCountArg.slice("--audit-count=".length) : undefined, DEFAULT_AUDIT_LOG_COUNT))
        ),
    };
}
...
function allocateByRatios<K extends string>(
    ...
    const allocatedSum = (Object.values(allocation) as number[]).reduce((sum, value) => sum + value, 0);
...
function sampleTimestampInRange(
    ...
    for (let attempt = 0; attempt < 24; attempt += 1) {
        ...
        candidate.setUTCHours(hour, randomInt(rng, 0, 59), randomInt(rng, 0, 59), randomInt(rng, 0, 999));
...
async function seedUsers(rng: () => number): Promise<SeedUserRecord[]> {
    ...
    assertCondition(usernames.length === TOTAL_USERS, `user count mismatch: expected ${TOTAL_USERS}, got ${usernames.length}`);

    const now = NOW;
    const userRows = usernames.map((username) => {
        const group: SeedUserGroup = ADMIN_USERNAMES.includes(username as (typeof ADMIN_USERNAMES)[number])
            ? "admin"
            : PENTEST_USERNAMES.includes(username as (typeof PENTEST_USERNAMES)[number])
                ? "pentest"
                : "general";
        const userRole: "admin" | "user" = group === "admin" ? "admin" : "user";
        const createdAt = sampleTimestampInRange(rng, USER_START_DATE, USER_END_DATE, { recencyBias: 1.35 });
        ...
        return {
            ...
            updated_at: new Date(Math.max(createdAt.getTime(), now.getTime() - randomInt(rng, 1, 30) * 86_400_000)),
        };
    });
    ...
    return inserted.map((row) => {
        const group: SeedUserGroup = ADMIN_USERNAMES.includes(row.username as (typeof ADMIN_USERNAMES)[number])
            ? "admin"
...
function makeContent(rng: () => number, language: LanguageKind, lengthKind: LengthKind, boardSlug: BoardSlug): string {
    const basePool = language === "en" ? ENGLISH_CONTENT_LINES : KOREAN_CONTENT_LINES;

    const lineCount = lengthKind === "short"
        ? randomInt(rng, 2, 3)
        : lengthKind === "medium"
            ? randomInt(rng, 4, 5)
            : randomInt(rng, 7, 9);
...
async function seedPosts(params: {
    ...
    const slugByBoardId = new Map<number, BoardSlug>(boards.map((board) => [board.boardId, board.slug]));

    const stored = storedRows.map((row) => {
        const slug = slugByBoardId.get(Number(row.board_id));
        assertCondition(typeof slug === "string", `missing board slug for board_id=${String(row.board_id)}`);
...
async function attachProfileAvatars(params: {
    ...
    const adminOrPentestIds = new Set(users.filter((user) => user.group !== "general").map((user) => user.userId));
    await sequelize.query(
        `
        UPDATE users
...
function buildSessionSlots(params: {
    ...
    const buildRangeSlots = (targetEvents: number, rangeStart: Date, rangeEnd: Date, recencyBias: number) => {
        let produced = 0;
    ...
    buildRangeSlots(totalEvents - recentTarget, AUDIT_START_DATE, new Date(recentStart.getTime() - 1), 1.15);
...
function buildAuditDetails(params: {
    ...
    if (activity === "login_fail") {
        ...
        details.reason = pickOne(rng, ["invalid_password", "invalid_username", "captcha_required"] as const);
    }
    ...
    if (activity === "password_reset_requested") {
        ...
        details.requestedUsername = slot.actor?.username ?? pickOne(rng, [...GENERAL_USERNAMES, ...PENTEST_USERNAMES]);
    }
...
async function seedAuditLogs(params: {
    ...
    for (const slot of slots) {
        ...
        if ((activity === "login_success" || activity === "logout" || activity === "post_create" || activity === "post_update" || activity === "post_delete") && !actor) {
            actor = pickOne(rng, users);
        }

        let target: SeedUserRecord | null = null;
        if (activity === "login_success" || activity === "logout" || activity === "password_reset_completed" || activity === "account_locked") {
            target = actor;
...
async function loadRawAssets(repoRoot: string): Promise<{
    ...
    const avatarRawPaths = (await listFilesAbsolute(avatarDir))
        .filter((filePath) => path.extname(filePath).toLowerCase() === ".webp");

    const postImageRawPaths = (await listFilesAbsolute(postImageDir))
        .filter((filePath) => [".jpg", ".jpeg", ".png", ".webp"].includes(path.extname(filePath).toLowerCase()));

    const allowedExtensions = new Set(Array.from(ARTICLE_ATTACHMENT_EXTENSIONS).map((ext) => ext.toLowerCase()));
    const fileRawPaths = (await listFilesAbsolute(fileDir))
    ...
    assertCondition(postImageRawPaths.length > 0, "no post-image raw files found in seed-assets/raw/post-images");
    assertCondition(fileRawPaths.length > 0, "no allowed attachment raw files found in seed-assets/raw/files");
...
async function validateDistributionGuards(): Promise<void> {
    ...
    assertCondition(Number(postTotal?.count ?? 0) === TOTAL_POSTS, `post total mismatch after seeding: ${postTotal?.count ?? "0"}`);

    const [userTotal] = await sequelize.query<{ count: string }>(
        `SELECT COUNT(*)::text AS count FROM users`,
        { type: QueryTypes.SELECT }
    );
    assertCondition(Number(userTotal?.count ?? 0) === TOTAL_USERS, `user total mismatch after seeding: ${userTotal?.count ?? "0"}`);
    ...
    for (const [slug, expected] of Object.entries(expectedBoard)) {
        ...
        assertCondition(actual === expected, `board count mismatch for ${slug}: expected=${expected}, actual=${actual}`);
    }
    ...
    assertCondition(Number(attachmentGuard?.too_many_images ?? 0) === 0, "invalid multi-image representation detected");
    assertCondition(Number(attachmentGuard?.too_many_files ?? 0) === 0, "invalid multi-file representation detected");
}
