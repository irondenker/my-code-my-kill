...
test(
  'avatar controller rejects unsupported mime and supports upload/delete flow',
  { skip: skipReason },
  async () => {
    ...
    try {
      ...
      await withTestServer(async (baseUrl) => {
        ...
        uploadedPath = uploadedName
          ? path.join(AVATAR_IMAGE_UPLOAD_DIR, path.basename(uploadedName))
          : null;
        if (!uploadedPath) {
...
test(
  'avatar controller validates missing file, magic number, file size, and image dimensions',
  { skip: skipReason },
  async () => {
    ...
    try {
      ...
      await withTestServer(async (baseUrl) => {
        ...
        invalidMagicForm.set(
          'avatar',
          new Blob([Uint8Array.from(Buffer.from('plain-text-not-image', 'utf8'))], {
            type: 'image/png',
          }),
          'avatar.png'
        );
        ...
        const tooLargeBytes = addPngSignaturePrefix(
          Uint8Array.from(Buffer.alloc(AVATAR_IMAGE_MAX_BYTES + 1, 0))
        );
        const tooLargeForm = new FormData();
        tooLargeForm.set('_csrf', tooLargePage.csrfToken);
        tooLargeForm.set(
          'avatar',
          new Blob([Buffer.from(tooLargeBytes)], { type: 'image/png' }),
          'avatar.png'
        );
        ...
        tooWideForm.set(
          'avatar',
          new Blob([Uint8Array.from(tooWideBuffer)], { type: 'image/png' }),
          'avatar.png'
        );
        ...
        tooSmallForm.set(
          'avatar',
          new Blob([Uint8Array.from(tooSmallBuffer)], { type: 'image/png' }),
          'avatar.png'
        );
...
test(
  'user profile page exposes private fields only to owner or admin',
  { skip: skipReason },
  async () => {
...
test(
  'public profile route returns 404 when username does not exist',
  { skip: skipReason },
  async () => {
    await withTestServer(async (baseUrl) => {
...
test(
  'user profile image url is normalized for stored filename and preserved for absolute upload path',
  { skip: skipReason },
  async () => {
    ...
    try {
      ...
      await withTestServer(async (baseUrl) => {
        ...
        assert.equal(
          prefixedBody.includes('/uploads/avatars//uploads/avatars/already-prefixed.webp'),
          false
        );
      });
    } finally {
...
test(
  'profile edit validates display name/email/phone/bio and returns 422',
  { skip: skipReason },
  async () => {
...
test(
  'profile edit updates profile and redirects to public profile page',
  { skip: skipReason },
  async () => {
...
test(
  'settings profile page returns 404 when authenticated session user row is missing',
  { skip: skipReason },
  async () => {
