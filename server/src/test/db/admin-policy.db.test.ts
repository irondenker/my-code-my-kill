...
test(
  'admin controller blocks self role revoke and validates board slug',
  { skip: skipReason },
  async () => {
    ...
    try {
      ...
      await withTestServer(async (baseUrl) => {
        ...
        const validSlug = makeId('p1board')
          .replace(/[^a-z0-9-]/g, '')
          .toLowerCase()
          .slice(0, 24);
...
test(
  'audit logs page normalizes limit query and selected option',
  { skip: skipReason },
  async () => {
...
test(
  'admin board edit handles 422/404/409 and successful update',
  { skip: skipReason },
  async () => {
    const adminUsername = makeId('boardedit-admin').slice(0, 32);
    const adminPassword = 'board-edit-pass-123';

    const originalSlug = makeId('edit-src')
      .replace(/[^a-z0-9-]/g, '')
      .toLowerCase()
      .slice(0, 24);
    const conflictSlug = makeId('edit-conf')
      .replace(/[^a-z0-9-]/g, '')
      .toLowerCase()
      .slice(0, 24);
    const nextSlug = makeId('edit-next')
      .replace(/[^a-z0-9-]/g, '')
      .toLowerCase()
      .slice(0, 24);
    ...
    try {
      ...
      await withTestServer(async (baseUrl) => {
        ...
        const invalidEnumResponse = await fetch(
          `${baseUrl}/admin/boards/${sourceBoard.boardId}/edit`,
          {
            method: 'POST',
            headers: {
              'content-type': 'application/x-www-form-urlencoded',
...
test(
  'admin user status/role endpoints handle invalid value, no-change flash, and missing target',
  { skip: skipReason },
  async () => {
    ...
    try {
      ...
      await withTestServer(async (baseUrl) => {
        ...
        const invalidStatusResponse = await fetch(
          `${baseUrl}/admin/users/${targetUser.userId}/status`,
          {
            method: 'POST',
            headers: {
              'content-type': 'application/x-www-form-urlencoded',
        ...
        const invalidRoleResponse = await fetch(
          `${baseUrl}/admin/users/${targetUser.userId}/role`,
          {
            method: 'POST',
            headers: {
              'content-type': 'application/x-www-form-urlencoded',
        ...
        const noChangeStatusResponse = await fetch(
          `${baseUrl}/admin/users/${targetUser.userId}/status`,
          {
            method: 'POST',
            headers: {
        ...
        const noChangeRoleResponse = await fetch(
          `${baseUrl}/admin/users/${targetUser.userId}/role`,
          {
            method: 'POST',
            headers: {
...
test(
  'admin board create validates required fields, length, enum, and duplicate slug',
  { skip: skipReason },
  async () => {
    const adminUsername = makeId('boardcreate-admin').slice(0, 32);
    const adminPassword = 'board-create-pass-123';
    const duplicateSlug = makeId('dup-board')
      .replace(/[^a-z0-9-]/g, '')
      .toLowerCase()
      .slice(0, 24);
    ...
    try {
      ...
      await withTestServer(async (baseUrl) => {
        ...
        const cases: Array<{
          ...
          {
            body: `slug=${encodeURIComponent(
              makeId('name-len')
                .replace(/[^a-z0-9-]/g, '')
                .toLowerCase()
                .slice(0, 24)
            )}&name=${encodeURIComponent('N'.repeat(101))}&description=&readAccess=public&createAccess=auth`,
            expectedStatus: 422,
          ...
          },
          {
            body: `slug=${encodeURIComponent(
              makeId('desc-len')
                ...
                .toLowerCase()
                .slice(0, 24)
            )}&name=${encodeURIComponent('Desc Length Board')}&description=${encodeURIComponent('d'.repeat(256))}&readAccess=public&createAccess=auth`,
            expectedStatus: 422,
          ...
          },
          {
            body: `slug=${encodeURIComponent(
              makeId('bad-read')
                ...
                .toLowerCase()
                .slice(0, 24)
            )}&name=${encodeURIComponent('Bad Read Board')}&description=&readAccess=owner&createAccess=auth`,
            expectedStatus: 422,
          ...
          },
          {
            body: `slug=${encodeURIComponent(
              makeId('bad-create')
                ...
                .toLowerCase()
                .slice(0, 24)
            )}&name=${encodeURIComponent('Bad Create Board')}&description=&readAccess=public&createAccess=user`,
            expectedStatus: 422,
...
test('admin dashboard renders stats from live counts', { skip: skipReason }, async () => {
  ...
  const boardSlug = makeId('dash-board')
    .replace(/[^a-z0-9-]/g, '')
    .toLowerCase()
    .slice(0, 24);
...
test(
  'audit logs page renders for admin without mutating global audit state',
  { skip: skipReason },
  async () => {
