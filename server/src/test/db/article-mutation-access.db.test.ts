...
test(
  'article edit/delete routes enforce ownership and status branches',
  { skip: skipReason },
  async () => {
    ...
    const boardSlug = makeId('mut-board')
      .replace(/[^a-z0-9-]/g, '')
      .toLowerCase()
      .slice(0, 24);
    ...
    try {
      ...
      await withTestServer(async (baseUrl) => {
        ...
        const invalidEditResponse = await fetch(
          `${baseUrl}/board/${boardSlug}/${first.displayId}/edit`,
          {
            method: 'POST',
            headers: {
        ...
        const validEditResponse = await fetch(
          `${baseUrl}/board/${boardSlug}/${first.displayId}/edit`,
          {
            method: 'POST',
            headers: {
        ...
        assert.equal(
          validEditResponse.headers.get('location'),
          `/board/${boardSlug}/${first.displayId}`
        );
        ...
        const secondShowAfterDelete = await fetch(
          `${baseUrl}/board/${boardSlug}/${second.displayId}`,
          {
            headers: { cookie: ownerCookie },
            redirect: 'manual',
          }
        ...
        const ownerDeletePost = await fetch(
          `${baseUrl}/board/${boardSlug}/${first.displayId}/delete`,
          {
            method: 'POST',
            headers: {
              'content-type': 'application/x-www-form-urlencoded',
...
test(
  'article controller returns 404 for missing board or missing post edit target',
  { skip: skipReason },
  async () => {
    const username = makeId('missing-article').slice(0, 32);
    const password = 'missing-article-pass-123';
    const existingBoardSlug = makeId('exists-board')
      .replace(/[^a-z0-9-]/g, '')
      .toLowerCase()
      .slice(0, 24);
    const missingBoardSlug = makeId('missing-board')
      .replace(/[^a-z0-9-]/g, '')
      .toLowerCase()
      .slice(0, 24);
    ...
    try {
      ...
      await withTestServer(async (baseUrl) => {
        ...
        const missingPostResponse = await fetch(
          `${baseUrl}/board/${existingBoardSlug}/999999/edit`,
          {
            headers: { cookie: authCookie },
            redirect: 'manual',
          }
...
test(
  'article create/edit return 422 when upload validation throws ArticleUploadError',
  { skip: skipReason },
  async () => {
    ...
    const boardSlug = makeId('upload-err-board')
      .replace(/[^a-z0-9-]/g, '')
      .toLowerCase()
      .slice(0, 24);
    ...
    try {
      ...
      await withTestServer(async (baseUrl) => {
        ...
        const badAttachmentBytes = Uint8Array.from(
          Buffer.from('not-a-supported-attachment', 'utf8')
        );
        ...
        createPayload.set(
          'attachment',
          new Blob([badAttachmentBytes], { type: 'application/octet-stream' }),
          'malware.bin'
        );
        ...
        const editResponse = await fetch(
          `${baseUrl}/board/${boardSlug}/${created.displayId}/edit`,
          {
            method: 'POST',
            headers: {
