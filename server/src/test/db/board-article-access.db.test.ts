...
test(
  'board/article controllers enforce read/create access matrix for anon/user/admin',
  { skip: skipReason },
  async () => {
    ...
    const publicSlug = makeId('public-board')
      .replace(/[^a-z0-9-]/g, '')
      .toLowerCase()
      .slice(0, 24);
    const authSlug = makeId('auth-board')
      .replace(/[^a-z0-9-]/g, '')
      .toLowerCase()
      .slice(0, 24);
    const adminSlug = makeId('admin-board')
      .replace(/[^a-z0-9-]/g, '')
      .toLowerCase()
      .slice(0, 24);
    ...
    try {
      ...
      await withTestServer(async (baseUrl) => {
        ...
        assert.equal(
          anonAuthRead.headers.get('location'),
          `/login?next=${encodeURIComponent(`/board/${authSlug}`)}`
        );

        const anonAdminRead = await fetch(`${baseUrl}/board/${adminSlug}`, { redirect: 'manual' });
        assert.equal(anonAdminRead.status, 302);
        assert.equal(
          anonAdminRead.headers.get('location'),
          `/login?next=${encodeURIComponent(`/board/${adminSlug}`)}`
        );
...
test(
  'owner_or_admin board masks list and enforces show access by ownership/admin',
  { skip: skipReason },
  async () => {
    ...
    const boardSlug = makeId('owneronly')
      .replace(/[^a-z0-9-]/g, '')
      .toLowerCase()
      .slice(0, 24);
    ...
    try {
      ...
      await withTestServer(async (baseUrl) => {
        const anonShow = await fetch(`${baseUrl}/board/${boardSlug}/${ownerPost.displayId}`, {
          redirect: 'manual',
        });
        assert.equal(anonShow.status, 302);
        assert.equal(
          anonShow.headers.get('location'),
          `/login?next=${encodeURIComponent(`/board/${boardSlug}/${ownerPost.displayId}`)}`
        );
...
test('board controller returns 404 for unknown board slug', { skip: skipReason }, async () => {
  await withTestServer(async (baseUrl) => {
    const missingSlug = makeId('missing-board')
      .replace(/[^a-z0-9-]/g, '')
      .toLowerCase()
      .slice(0, 24);
...
test(
  'owner_or_admin board is hidden from anon board directory but visible to authenticated users',
  { skip: skipReason },
  async () => {
    const username = makeId('owner-dir-user').slice(0, 32);
    const password = 'owner-dir-user-pass-123';
    const boardSlug = makeId('owner-dir')
      .replace(/[^a-z0-9-]/g, '')
      .toLowerCase()
      .slice(0, 24);
