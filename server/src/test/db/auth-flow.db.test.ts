...
function cloneSecurityDefenseLabOptions(
  value: SecurityDefenseLabOptions
): SecurityDefenseLabOptions {
...
async function postResetPasswordForm(params: {
  ...
  const response = await fetch(
    `${params.baseUrl}/reset-password?token=${encodeURIComponent(params.token)}`,
    {
      method: 'POST',
      headers: {
        'content-type': 'application/x-www-form-urlencoded',
...
test(
  'auth endpoints register successfully with valid csrf/session and persist user',
  { skip: skipReason },
  async () => {
    ...
    try {
      await withTestServer(async (baseUrl) => {
        ...
        const nextCookie =
          response.headers.get('set-cookie')?.match(/mcmk\.sid=[^;]+/)?.[0] ?? cookie;
...
test(
  'auth endpoints login successfully and honor safe next redirect',
  { skip: skipReason },
  async () => {
...
test(
  'auth endpoints fallback to / for unsafe next on login success',
  { skip: skipReason },
  async () => {
...
test(
  'auth endpoints logout clears session and redirects to root',
  { skip: skipReason },
  async () => {
...
test(
  'auth defense resets failed counter after successful login when enabled',
  { skip: skipReason },
  async () => {
...
test(
  'auth defense enforces password reset after five failed logins',
  { skip: skipReason },
  async () => {
...
test(
  'forgot/reset flow clears reset-required state and updates password',
  { skip: skipReason },
  async () => {
...
test(
  'forgot-password returns generic response and shows only reset link',
  { skip: skipReason },
  async () => {
