...
async function withDeterministicUploadNames<T>(baseTimestamp: number, run: () => Promise<T>): Promise<T> {
    ...
    cryptoRef.randomBytes = ((size: number) => Buffer.from(FIXED_UPLOAD_RANDOM_HEX.repeat(Math.ceil(size / 8)).slice(0, size * 2), "hex")) as typeof crypto.randomBytes;
...
test("createArticleWithUploads cleans stored files when DB create fails", { skip: skipReason }, async () => {
...
test("updateArticleWithUploads returns false and rolls back new uploads for missing post", { skip: skipReason }, async () => {
...
test("updateArticleWithUploads deletes replaced old files after successful update", { skip: skipReason }, async () => {
    const username = makeId("upreplace").slice(0, 32);
    const boardSlug = makeId("upreplace-board").replace(/[^a-z0-9-]/g, "").toLowerCase().slice(0, 30);
...
test("createArticle allocates unique sequential display ids under concurrent writes", { skip: skipReason }, async () => {
    const username = makeId("conwriter").slice(0, 32);
    const boardSlug = makeId("conboard").replace(/[^a-z0-9-]/g, "").toLowerCase().slice(0, 24);
    let userId: number | null = null;
    let boardId: number | null = null;
    ...
    try {
        ...
        assert.deepEqual(displayIds, Array.from({ length: batchSize }, (_, i) => i + 1));
...
test("findNeighborArticles handles first/last edges and owner filter", { skip: skipReason }, async () => {
    const ownerUsername = makeId("neighbor-owner").slice(0, 32);
    const otherUsername = makeId("neighbor-other").slice(0, 32);
    const boardSlug = makeId("neighbor-board").replace(/[^a-z0-9-]/g, "").toLowerCase().slice(0, 24);
