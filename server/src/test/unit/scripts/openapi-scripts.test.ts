...
test('check-openapi-drift script exits cleanly on current route/docs snapshot', async () => {
  await execFileAsync(process.execPath, ['--import', 'tsx', 'src/scripts/check-openapi-drift.ts'], {
    cwd: process.cwd(),
    env: { ...process.env },
...
test('sync-openapi-endpoints script writes placeholder operation for missing route in temp workspace', async () => {
  ...
  try {
    ...
    const syncScriptUrl = pathToFileURL(
      path.join(process.cwd(), 'src/scripts/sync-openapi-endpoints.ts')
    ).href;
