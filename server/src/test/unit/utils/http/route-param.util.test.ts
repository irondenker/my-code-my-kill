...
import {
  getPositiveIntParamOrThrow,
  getStringParamOrThrow,
} from '../../../../utils/http/route-param.util.js';
...
test('getStringParamOrThrow throws HttpError(404) for missing/empty param', () => {
  ...
  assert.throws(
    () => getStringParamOrThrow(req as any, 'slug'),
    (err: unknown) => {
...
test('getPositiveIntParamOrThrow throws HttpError(404) for invalid numbers', () => {
  ...
  assert.throws(
    () => getPositiveIntParamOrThrow(req as any, 'displayId'),
    (err: unknown) => {
      assert.equal(err instanceof HttpError, true);
      assert.equal((err as HttpError).status, 404);
      return true;
