...
import {
  ...
  regenerateSession,
  saveSession,
} from '../../../../utils/session/session.util.js';
...
test('regenerateSession rejects when callback returns an error', async () => {
  ...
  await assert.rejects(async () => regenerateSession(req), /regenerate failed/);
});
...
test('saveSession rejects when callback returns an error', async () => {
  ...
  await assert.rejects(async () => saveSession(req), /save failed/);
});
...
test('destroySession rejects when callback returns an error', async () => {
  ...
  await assert.rejects(async () => destroySession(req), /destroy failed/);
});
