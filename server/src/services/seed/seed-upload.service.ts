...
async function toMulterFileFromPath(
  rawPath: string,
  fieldname: string
...
export async function uploadProfileImageFromPath(
  userId: number,
  rawPath: string
): Promise<SeedStoredUpload> {
  const { uploadProfileImageFromFile } =
    await import('../profile/profile-avatar-upload.service.js');
...
export async function uploadPostImageFromPath(
  postId: number,
  rawPath: string
): Promise<SeedStoredUpload> {
