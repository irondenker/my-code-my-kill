...
import { buildViewerContext, getBoardReadAccessResult } from '../utils/board/board.policy.util.js';
import { findBoardBySlug, listBoards } from '../services/board.service.js';
import { countArticlesBySlug, listArticleOutlinesBySlug } from '../services/article.service.js';
 ...
 */
function canAccessBoardDirectory(
  req: Request,
  board: NonNullable<Awaited<ReturnType<typeof findBoardBySlug>>>
): boolean {
 ...
 */
function canCreateForBoard(
  req: Request,
  board: Awaited<ReturnType<typeof findBoardBySlug>>
): boolean {
  if (!board) {
    return false;
  }
...
export async function getBoardBySlug(req: Request, res: Response) {
  ...
  const queryCarry = (overrides: BoardListQueryOverrides = {}) =>
    buildBoardListQueryCarry(boardListQuery, overrides);
