...
export async function postProfileEdit(req: Request, res: Response) {
  ...
  const email = parsedProfileForm.success
    ? parsedProfileForm.data.email
  ...
  const bio = parsedProfileForm.success
    ? parsedProfileForm.data.bio
    : normalizeString(req.body?.bio, null);
