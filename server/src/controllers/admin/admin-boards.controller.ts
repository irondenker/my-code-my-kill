...
export async function postAdminBoardCreate(req: Request, res: Response) {
  ...
  const slug = parsedBoardForm.success
    ? parsedBoardForm.data.slug
    : normalizeString(req.body?.slug).toLowerCase();
  const name = parsedBoardForm.success
    ? parsedBoardForm.data.name
...
export async function postAdminBoardEdit(req: Request, res: Response) {
  ...
  if (!isValidBoardSlug(slug)) {
    return renderInvalid(
      'Slug must be 2-50 chars and use lowercase letters, numbers, hyphens only.'
    );
  }
