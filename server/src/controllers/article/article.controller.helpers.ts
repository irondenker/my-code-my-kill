...
export function ensureBoardCreateAccess(
  req: Request,
...
export function ensureBoardReadAccess(
  req: Request,
...
export function ensurePostEditAccess(
  req: Request,
  post: Pick<ArticleRecord, 'boardSlug' | 'userId'>
): ViewerContext {
...
export function requireAuthenticatedViewerId(viewerContext: ViewerContext): number {
  if (
    !viewerContext.isAuthenticated ||
    !Number.isFinite(viewerContext.viewerUserId) ||
    viewerContext.viewerUserId <= 0
  ) {
    throw new HttpError(401, 'Unauthorized');
  }
