...
function parseBooleanOption(value: unknown, key: string, fallback = false): boolean {
  ...
  if (typeof value !== 'undefined') {
    console.warn(
      `[CONFIG] Invalid lab option "${key}" in ${LAB_OPTIONS_PATH}. Using ${String(fallback)}.`
    );
  }
  return fallback;
}
...
function parseEscapeRuleList(value: unknown, key: string): EscapeRule[] {
  ...
  value.forEach((entry, index) => {
    if (!entry || typeof entry !== 'object' || Array.isArray(entry)) {
      console.warn(
        `[CONFIG] Invalid lab option "${key}[${index}]" in ${LAB_OPTIONS_PATH}. Expected object.`
      );
      return;
    }
    ...
    if (typeof rule.from !== 'string' || typeof rule.to !== 'string') {
      console.warn(
        `[CONFIG] Invalid lab option "${key}[${index}]" in ${LAB_OPTIONS_PATH}. "from" and "to" must be strings.`
      );
      return;
    }
    ...
    if (!rule.from) {
      console.warn(
        `[CONFIG] Invalid lab option "${key}[${index}]" in ${LAB_OPTIONS_PATH}. "from" cannot be empty.`
      );
      return;
    }
...
function parseXssSideOptions(value: unknown, keyPrefix: string): XssSideOptions {
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    console.warn(
      `[CONFIG] Invalid lab option "${keyPrefix}" in ${LAB_OPTIONS_PATH}. Using defaults.`
    );
    return cloneDefaultXssSideOptions();
  }
  ...
  const parsedToggles =
    rawToggles && typeof rawToggles === 'object' && !Array.isArray(rawToggles)
      ...
      : {};
  if (
    typeof rawToggles !== 'undefined' &&
    (!rawToggles || typeof rawToggles !== 'object' || Array.isArray(rawToggles))
  ) {
    console.warn(
      `[CONFIG] Invalid lab option "${keyPrefix}.defaultRuleToggles" in ${LAB_OPTIONS_PATH}. Using defaults.`
    );
  }
...
function parseXssInjectionOptions(value: unknown): XssInjectionOptions {
  ...
  const parsedStored =
    rawStored && typeof rawStored === 'object' && !Array.isArray(rawStored)
      ...
      : {};
  if (
    typeof rawStored !== 'undefined' &&
    (!rawStored || typeof rawStored !== 'object' || Array.isArray(rawStored))
  ) {
    console.warn(
      `[CONFIG] Invalid lab option "xss.stored" in ${LAB_OPTIONS_PATH}. Using defaults.`
    );
  }
  ...
  if (
    ...
    console.warn(
      `[CONFIG] Invalid lab option "xss.reflected404" in ${LAB_OPTIONS_PATH}. Using defaults.`
    );
  }
  ...
  const parsedSanitize =
    rawSanitize && typeof rawSanitize === 'object' && !Array.isArray(rawSanitize)
      ...
      : {};
  if (
    typeof rawSanitize !== 'undefined' &&
    (!rawSanitize || typeof rawSanitize !== 'object' || Array.isArray(rawSanitize))
  ) {
    console.warn(
      `[CONFIG] Invalid lab option "xss.sanitize" in ${LAB_OPTIONS_PATH}. Using defaults.`
    );
  }
...
function parseUploadValidationOptions(value: unknown): UploadValidationOptions {
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    console.warn(
      `[CONFIG] Invalid lab option "uploadValidation" in ${LAB_OPTIONS_PATH}. Using defaults.`
    );
    return getDefaultUploadValidationOptions();
  }
  ...
  const parsedDefaultRuleToggles =
    rawDefaultRuleToggles &&
    typeof rawDefaultRuleToggles === 'object' &&
    !Array.isArray(rawDefaultRuleToggles)
  ...
  const extensionCheckRaw =
    typeof options.extensionCheck === 'undefined'
      ? parsedDefaultRuleToggles.extensionCheck
      : options.extensionCheck;
  const mimeCheckRaw =
    typeof options.mimeCheck === 'undefined'
      ? parsedDefaultRuleToggles.mimeCheck
      : options.mimeCheck;
  ...
  return {
    ...
    mimeCheck: parseBooleanOption(mimeCheckRaw, 'uploadValidation.mimeCheck', defaults.mimeCheck),
...
function parseDebugErrorRoutesOption(value: unknown): boolean {
  ...
  const parsedErrorRoutes =
    rawErrorRoutes && typeof rawErrorRoutes === 'object' && !Array.isArray(rawErrorRoutes)
      ...
      : {};
  if (
    typeof rawErrorRoutes !== 'undefined' &&
    (!rawErrorRoutes || typeof rawErrorRoutes !== 'object' || Array.isArray(rawErrorRoutes))
  ) {
    console.warn(
      `[CONFIG] Invalid lab option "debug.errorRoutes" in ${LAB_OPTIONS_PATH}. Using defaults.`
    );
  }
...
function parseSqlInjectionTargetOption(params: {
  ...
  return parseBooleanOption(
    params.parsedTargets?.[params.targetKey],
    `sqlInjection.targets.${params.targetKey}`,
    fallback
  );
}
...
function parseSqlInjectionOptions(value: unknown): SqlInjectionOptions {
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    console.warn(
      `[CONFIG] Invalid lab option "sqlInjection" in ${LAB_OPTIONS_PATH}. Using defaults.`
    );
    return {
      enabled: DEFAULT_LAB_OPTIONS.sqlInjection.enabled,
      targets: { ...DEFAULT_LAB_OPTIONS.sqlInjection.targets },
  ...
  if (typeof rawTargets !== 'undefined' && !parsedTargets) {
    console.warn(
      `[CONFIG] Invalid lab option "sqlInjection.targets" in ${LAB_OPTIONS_PATH}. Using defaults.`
    );
  }
...
function parseCsrfOptions(value: unknown): CsrfOptions {
  ...
  const options = value as Record<string, unknown>;
  return {
    enabled: parseBooleanOption(options.enabled, 'csrf.enabled', DEFAULT_LAB_OPTIONS.csrf.enabled),
  };
}
...
function parseCspOptions(value: unknown): CspOptions {
  ...
  const options = value as Record<string, unknown>;
  return {
    enabled: parseBooleanOption(options.enabled, 'csp.enabled', DEFAULT_LAB_OPTIONS.csp.enabled),
  };
}
...
function parseSstiOption(value: unknown): boolean {
  ...
  const options = value as Record<string, unknown>;
  return parseBooleanOption(options.enabled, 'ssti.enabled', DEFAULT_LAB_OPTIONS.ssti);
}
...
function parseSecurityDefenseLabOptions(value: unknown): SecurityDefenseLabOptions {
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    console.warn(
      `[CONFIG] Invalid lab option "securityDefense" in ${LAB_OPTIONS_PATH}. Ignoring values.`
    );
    return {};
  }
  ...
  if (typeof rawAccountLockout !== 'undefined' && !parsedAccountLockout) {
    console.warn(
      `[CONFIG] Invalid lab option "securityDefense.accountLockout" in ${LAB_OPTIONS_PATH}. Ignoring values.`
    );
  }
  ...
  if (typeof rawPasswordReset !== 'undefined' && !parsedPasswordReset) {
    console.warn(
      `[CONFIG] Invalid lab option "securityDefense.passwordReset" in ${LAB_OPTIONS_PATH}. Ignoring values.`
    );
  }
  ...
  if (typeof rawRateLimit !== 'undefined' && !parsedRateLimit) {
    console.warn(
      `[CONFIG] Invalid lab option "securityDefense.rateLimit" in ${LAB_OPTIONS_PATH}. Ignoring values.`
    );
  }
  if (parsedRateLimit) {
  ...
  if (typeof rawSimpleCaptcha !== 'undefined' && !parsedSimpleCaptcha) {
    console.warn(
      `[CONFIG] Invalid lab option "securityDefense.simpleCaptcha" in ${LAB_OPTIONS_PATH}. Ignoring values.`
    );
  }
  if (parsedSimpleCaptcha) {
    ...
    const parsedLoginSimpleCaptcha =
      rawLoginSimpleCaptcha &&
      typeof rawLoginSimpleCaptcha === 'object' &&
      !Array.isArray(rawLoginSimpleCaptcha)
        ? (rawLoginSimpleCaptcha as Record<string, unknown>)
        : null;
    if (typeof rawLoginSimpleCaptcha !== 'undefined' && !parsedLoginSimpleCaptcha) {
      console.warn(
        `[CONFIG] Invalid lab option "securityDefense.simpleCaptcha.login" in ${LAB_OPTIONS_PATH}. Ignoring values.`
      );
    }
    if (parsedLoginSimpleCaptcha) {
      const loginSimpleCaptcha: NonNullable<
        NonNullable<SecurityDefenseLabOptions['simpleCaptcha']>['login']
      > = {};
